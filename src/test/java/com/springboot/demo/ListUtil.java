...
public class ListUtil {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            //双色球随机生成器
            Set<String> redNbr = new TreeSet<String>();
            while (true) {
                int sui = (int) (Math.random() * 33) + 1;// 获取33以内的数字
                redNbr.add(sui < 10 ? "0" + sui : "" + sui);//将元素存入集合中
                if (redNbr.size() == 6) {//存满六个红色球，则结束循环
                    break;
                }
            }
            Set<String> blueNbr = new TreeSet<String>();
            int sui2 = (int) (Math.random() * 16) + 1;// 获取16以内的数字
            blueNbr.add(sui2 < 10 ? "0" + sui2 : "" + sui2);
            System.out.println("红球：" + redNbr + " 蓝球：" + blueNbr);
        }
